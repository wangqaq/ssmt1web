layui.use(['form', 'upload','layer', 'jquery', 'laydate','layedit'],
    function () {
        ...
        layedit.set({
            //默认post
            uploadImage: {
                url: '/api/picture/upload/type=mail/id='+sessionStorage.getItem("id") //接口url
            }
        });
        ...
        form.on('submit(edit)',
            function (data) {
                ...
                data.callBack = layedit.getContent(index);
                let res = myAjax("/api/mail/callBack", data);
        ...
        $(function () {
            ...
            let data = myAjax("/api/mail/findById", {id: id}, 'get');
            console.log(data);
            if (data.code===0){
                setData(data.data);
                setText(data.data);
        ...
        function setText(data) {
            layedit.setContent(index,data.callBack);
        }
